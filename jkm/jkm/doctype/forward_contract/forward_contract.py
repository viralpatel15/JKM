...
class ForwardContract(Document):
	...
	def calculate_underlying(self):
		for row in self.forward_contract_underlying:
		...
		self.db_set('total_underlying', sum([flt(row.amount_covered) for row in self.forward_contract_underlying]))
	...
	def calculate_days_of_premium(self):
		if self.hedge == "Export":
			if self.maturity_from:
				...
				self.days_for_premium = days_for_premium.days
			else:
	...
	def set_status(self):
		...
		self.outstanding_inr = self.amount_outstanding * self.booking_rate
