		...
		if not meis_receivable_account:
			...
			for row in self.rodtep_details:
				meis_jv.append("accounts", {
					...
					"credit_in_account_currency":round(row.debit_amount,2),
				})
			meis_jv.append("accounts", {
				"account": self.credit_account,
				"debit_in_account_currency":round(self.script_amount,2),
			})
			if self.round_off_amount < 0.0:
				meis_jv.append("accounts", {
					"account": self.round_off_account,
					"credit_in_account_currency":round(-flt(self.round_off_amount),4),
				})
			elif self.round_off_amount > 0.0:
				meis_jv.append("accounts", {
					"account": self.round_off_account,
					"debit_in_account_currency":round(flt(self.round_off_amount),4),
				})
